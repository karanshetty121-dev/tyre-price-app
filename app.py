...
# 1. Standard Page Configuration (No Branding/Sticky Headers)
...
if check_password():
    # Standard Search Bar
    search = st.text_input("🔍 Global Search (Size or Pattern):", placeholder="e.g. 145 80 12 or Earth-1...")
    ...
    bridgestone_master = {
        ...
        "Type": ["TL"] * 21,
        "Price (YRP)": [2850, 3300, 3200, 3550, 3550, 3050, 3450, 3500, 4050, 3650, 3800, 3800, 4000, 4000, 4150, 4300, 4450, 4450, 4500, 4200, 4850],
        "MRP": [3061, 3544, 3450, 3825, 3783, 3293, 3685, 3776, 4322, 3940, 4089, 4080, 4274, 4296, 4452, 4623, 4789, 4801, 4820, 4520, 5234]
    }

    # --- BALANCED YOKOHAMA DATA (From PDF Page 1) ---
    yokohama_master = {
        "Brand": ["Yokohama"] * 21,
        "Pattern": ["Earth-1 Max"] * 8 + ["Earth-1 Max"] * 10 + ["GT Max", "Earth-1 Max", "AS01"],
        "Tyre Size": [
            "145/80 R12 74T", "145/80 R13 75S", "155/65 R13 73T", "155/70 R13 75T", "155/80 R13 79T", "165/65 R13 77T", "175/60 R13 77H", "175/70 R13 82H", # Rim 12-13
            "155/65 R14 75H", "165/65 R14 79T", "165/70 R14 81T", "165/80 R14 85T", "175/65 R14 82H", "175/70 R14 84T", "175/70 R14 84H", "185/60 R14 82H", "185/60 R14 82H", "185/65 R14 86H", # Rim 14
            "185/70 R14 88H", "185/70 R14 88H", "195/60 R14 86H" # End of snippet
        ],
        "Type": ["TL"] * 21,
        "Price (YRP)": [3130, 3660, 3890, 4020, 4340, 4070, 4680, 5120, 3970, 4790, 4130, 4420, 5570, 5530, 5630, 5400, 6200, 5850, 5400, 6040, 6030],
        ...
        if not filt.empty:
            ...
            st.dataframe(filt, use_container_width=True, hide_index=True,
                column_config={
                    ...
                    "Price (YRP)": st.column_config.NumberColumn("Price (₹)", format="₹%d"),
                    "MRP": st.column_config.NumberColumn("MRP (₹)", format="₹%d")
                })
            st.markdown("---")

    show_table("Bridgestone Master List", bridgestone_master)
    show_table("Yokohama Master List", yokohama_master)
