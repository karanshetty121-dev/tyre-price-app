...
# 2. Custom CSS for Sticky Header & Branding
st.markdown("""
    <style>
...
with header:
    ...
    with col1:
        # High resolution MOTO FINEZ branding alignment
    ...
    # Global Sticky Search Bar (Searches Size and Pattern)
    search = st.text_input("🔍 Global Search:", placeholder="Enter size (e.g. 145 80 12) or pattern (e.g. Sturdo)...", key="master_search")
...
if check_password():
    ...
    # Types are specified properly (Tubeless, Tube Type, etc.)
    bridgestone_master = {
        "Category": (
            ["Passenger (BSID)"] * 152 + ["Run-Flat (RFT)"] * 20 + 
            ["Premium (T6i)"] * 53 + ["SUV (Dueler)"] * 10 + ["Commercial (LT)"] * 7
        ),
        "Rim": [
        ...
        "Consumer Price": [
            ...
            11150, 14550, 14550, 15900, 15900, 17100, 17100, 17150, 22750, 20500, 24050, 23350, 22650, 28550, 32900, 30700, 31500, 32900, 37850, 39650, # RFT
            5350, 6200, 6050, 6500, 6500, 6850, 6650, 6900, 7550, 7700, 8450, 7250, 9250, 8800, 9200, 10350, 8550, 8200, 11550, 9300, 10650, 12300, 11850, 10300, 10000, 10000, 10100, 12400, 12700, 12650, 12550, 12150, 14600, 20200, 13650, 13400, 20800, 21050, 15850, 21400, 16100, 19700, 19650, 21650, 21200, 26500, 25250, 28800, 25500, 25450, 27500, 25850, 27650, # Turanza
            7950, 9850, 14450, 15000, 14900, 15750, 16550, 13100, 19350, 20600, # Dueler
            3400, 4250, 4850, 5800, 6750, 7200, 9650 # LT
        ],
        "MRP": [
