...
# 2. Custom CSS for Sticky Header, Widened Columns, and Logo Resolution
st.markdown("""
    <style>
...
with header:
    ...
    with col1:
        # Replace with high-resolution logo link if available
    ...
    # Global Sticky Search Bar (Searches Tyre Size and Pattern)
    search = st.text_input("🔍 Global Search:", placeholder="Enter brand, size, or pattern (e.g. Sturdo or 145 80 12)...", key="master_search")
...
if check_password():
    ...
    # --- COMPLETE BALANCED MASTER DATASET ---
    # Every column has exactly 239 rows (152 BSID + 20 RFT + 67 Yokohama)
    master_data = {
        "Brand": (["Bridgestone"] * 172) + (["Yokohama"] * 67),
        "Category/Pattern": (
            # Bridgestone Patterns [cite: 140, 150, 155]
        ...
        "MRP": [
            ...
            # Yokohama MRP (same as YRP) [cite: 170, 171, 172, 173]
            [3130, 3660, 3890, 4020, 4340, 4070, 4680, 5120, 3970, 4790, 4130, 4420, 5570, 5530, 5630, 5400, 6200, 5850, 5400, 6040, 6030, 6220, 6220, 5740, 7360, 5870, 6570, 6350, 5860, 5960, 5970, 6060, 6210, 6300, 6300, 7290, 6480, 6880, 6770, 6950, 6940, 7050, 8160, 7320, 7850, 7380, 7430, 7070, 7180, 10430, 8520, 7980, 11020, 7720, 8470, 8920, 9860, 11900, 16140, 18280, 7500, 6760, 8340, 8490, 8520, 13350, 8130]
        ]
    }
    ...
    if search:
        ...
        mask = (df["Tyre Size"].str.contains(search, case=False)) | (df["Category/Pattern"].str.contains(search, case=False))
        filt = df[mask]
    else:
        filt = df

    st.subheader("📊 Master Data: Tyre Price List")
    st.dataframe(
        filt, use_container_width=True, hide_index=True,
        column_config={
            "Category/Pattern": st.column_config.Column(width="medium"),
